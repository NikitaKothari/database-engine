...
{
  private static final String ERROR = "Error, While fectching records from table";
  Utility utility = Utility.getInstance();
  
  public SelectWhere() {}
  
  public void selectWhere(String userCommand) { try { String[] tokens = userCommand.split(" ");
              ...
              for (Column column : columns) {
                if (column.getDataType().equals("int")) {
                  ...
                  if (column.getColumnName().equals(filterArray[0])) {
                    if (!value.equals(filterArray[1])) break;
